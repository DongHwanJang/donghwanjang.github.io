...
## Agent 1: Sarah Chen — Google Research, Research Intern Hiring Lead (3D Vision & Genertic AI Team)
...
3. **Model Stock (ECCV Oral)의 실용성** — Weight merging은 Google의 대규모 모델 운영과 직결되는 주제. 이 논문의 "24x less cost" 같은 효율성 메트릭이 Google의 DNA와 맞음.
...
2. **Weight space geometry에 대한 깊은 이해** — Model Stock과 Merge & Bound를 통해 fine-tuning dynamics, loss landscape, weight merging에 대한 비범한 이해도를 보여줌. 이건 LLaMA 같은 대형 모델의 fine-tuning 전략에 바로 적용 가능.
...
4. **Recommendation letters / Advisor network** — Ismini Lourentzou 교수의 NVIDIA 연결고리가 불분명. 이전 advisor인 Bohyung Han (SNU), Dongyoon Han/Sangdoo Yun (NAVER)의 네트워크는 주로 한국 중심.
...
> 아래는 세 명의 채용 담당자가 합동으로 도출한 홈페이지 + CV 개선사항입니다.
...
```
현재: 기술 스택 정보 없음
개선: Languages (Python, C++, Java), Frameworks (PyTorch, JAX),
     Tools (CUDA, Docker, Weights & Biases), Infra (Multi-GPU, Slurm)
     등을 명시적으로 나열
```
...
개선: 모든 논문에 코드 링크 추가. 없다면 reproduction code라도.
     개인 GitHub 프로필 링크도 추가.
     기여한 오픈소스 프로젝트가 있다면 반드시 포함.
```
...
개선: 자신만의 명확한 thesis statement 필요.
     예시: "I study how geometric priors in weight space and latent space
     can make generative models more efficient, controllable, and
     physically consistent — from model merging to reward-guided editing."
```
...
```
현재: HTML 주석으로 숨겨져 있음 (<!-- ... -->)
개선: 즉시 uncomment. TA 경험은 커뮤니케이션 능력의 증거.
     특히 "Samsung AI Expert Course", "Hyundai Motors AI Expert Course" TA는
     산업체 교육 경험이므로 매우 가치 있음.
```

### [High] 5. Work Experience를 홈페이지에 추가
```
현재: CV에만 존재, 홈페이지에 없음
개선: Samsung AI Center (Research Engineer) + NAVER AI (Research Intern) +
     Mind's Lab + TNC Technology를 timeline 형태로 홈페이지에 추가
```
...
개선: GitHub stars, citations, downloads 등 정량적 지표 추가
     ...
     예: "DS4C Dataset: 10,000+ Kaggle downloads, used in 100+ papers"
```

### [Medium] 7. Interactive Demo / Video 추가
```
현재: 정적 이미지만 있음
개선: RewardFlow의 image editing demo (Gradio/HuggingFace Spaces)
     PyraTok의 video reconstruction 예시 (embedded video)
     Model Stock의 interactive weight visualization
```
...
```
현재: 없음
개선: 논문의 핵심 아이디어를 비전문가도 이해할 수 있게 풀어 쓴
     블로그 포스트 1-2개. Medium 또는 자체 블로그.
```

### [Nice-to-have] 9. 웹사이트 현대화
```
현재: Jon Barron 템플릿 (기능적이지만 2010년대 스타일)
개선: 반응형 디자인, 다크모드, 더 나은 타이포그래피.
     학계에서는 괜찮지만, 산업체 지원 시에는 첫인상에 영향.
```
...
### [Critical] 1. Technical Skills 섹션 추가 (CV 1페이지 상단)
```
추가 위치: Research Interests 바로 아래
내용:
...
```
현재: "substantially reducing false positives" (Samsung)
개선: "reducing false positive rate by X%" — 구체적 숫자 필요
현재: "outperforming state-of-the-art methods" (Motion Deblur)
개선: "achieving X dB PSNR improvement over prior SOTA"
```

### [High] 3. Publication 순서 재정렬
```
현재: 1저자 논문이 맨 위 → 시간순이 아닌 중요도순 (좋음)
추가 개선: 각 논문의 contribution을 한 줄로 명시
  예: [1] ... — Proposed geometric analysis of weight space;
      24x more efficient than Model Soup
```

### [High] 4. "Software Engineer" 경력 활용법 개선
```
현재: "Developed a Java-based payment gateway server" (단순 기술)
개선: "Designed and deployed a high-availability payment gateway
      handling X transactions/day for Y corporate clients"
      — scale과 impact를 보여줄 것
```

### [Medium] 5. Reviewer / Service 경험 추가
```
학회 리뷰어 경험이 있다면 반드시 포함 (CVPR, ECCV, NeurIPS 등)
없다면 workshop organizer, mentoring 등의 service 활동
```

### [Medium] 6. 2페이지 포맷 최적화
```
현재: 2페이지이지만 여백이 많음
개선: Skills 섹션 추가 후에도 2페이지를 유지하되,
     Patent 정보를 별도 섹션으로 분리하여 가시성 향상
```
...
11. **"주어진 N개의 3D point cloud에서 가장 가까운 K개의 점을 찾는 알고리즘을 설계하세요."**
    - KD-tree, Ball tree, approximate NN (FAISS) 등

12. **"Image의 connected components를 효율적으로 찾는 알고리즘을 구현하세요."**
    - BFS/DFS, Union-Find

13. **"Distributed training에서 gradient synchronization을 구현하세요. All-reduce의 시간복잡도는?"**

### ML System Design (Meta/NVIDIA 스타일)
14. **"1000개의 fine-tuned 모델을 효율적으로 merge하는 시스템을 설계하세요."**
    - Model Stock을 scale-up한다면?

15. **"실시간 video editing 파이프라인을 설계하세요. Latency 요구사항은 100ms입니다."**
    - RewardFlow를 production에 넣는다면?

16. **"Wafer defect detection 시스템에서 false positive rate을 1% 이하로 낮추려면 어떻게 하시겠습니까?"**
    - Samsung 경험 기반
...
17. **"가장 실패했던 연구 경험은? 거기서 뭘 배웠나요?"**
    - Honesty, self-awareness, growth mindset 평가

18. **"Samsung에서 연구 결과를 production에 배포할 때 가장 큰 challenge는 뭐였나요?"**
    - Research-to-production gap 이해도

19. **"팀에서 의견 충돌이 있었을 때 어떻게 해결했나요?"**
    - NAVER AI Lab이나 CVPR 2026 공동 연구 경험 기반
...
> 기준: Dong-Hwan의 연구 분야 (generative AI, 3D/4D vision, model efficiency, diffusion models)와 매칭도,
> Ph.D. 1년차 적합성, 채용 현실성을 종합 고려하여 Tier 분류

## Tier 1: 최우선 지원 (Research Fit + 채용 가능성 높음)

| 회사 | 팀/포지션 | 매칭 이유 | 참고 |
|------|-----------|-----------|------|
| **NVIDIA Research** | Visual Computing / Generative AI Intern | Efficiency + generative model 조합이 NVIDIA 방향과 정확히 맞음. 특허 경험도 플러스. | 주로 가을에 모집, 3-6월 인턴 기간 |
| **Google DeepMind** | Research Intern (Generative Models) | Model efficiency, diffusion model 경험. Veo/Imagen 팀과 fit. | Host matching 시스템 — 교수 추천 중요 |
| **Google Research** | Student Researcher (Perception/3D) | 3D vision research interest + model merging 경험 | 미국 내 학생 우대, UIUC 위치 유리 |
| **Adobe Research** | Research Intern (GenAI/Creative AI) | RewardFlow의 image editing이 Adobe의 핵심 비즈니스와 직결 | 매년 10월-1월 모집 |
| **Microsoft Research** | Research Intern (Vision & Multimedia) | Broad CV 연구 범위가 MSR의 다양한 팀과 매칭 가능 | Redmond + NYC 오피스 |

## Tier 2: 강력 추천 (약간의 Gap 있지만 도전 가치 높음)

| 회사 | 팀/포지션 | 매칭 이유 | Gap |
|------|-----------|-----------|-----|
| **Meta FAIR** | Research Intern (Video Generation) | RewardFlow + PyraTok 경험. Make-A-Video, Emu 팀. | 1저자 video 논문 부재 |
| **Apple MLR** | AI/ML Research Intern | On-device efficiency 연구. Model Stock의 경량화 철학과 맞음. | Apple의 보수적 채용 프로세스 |
| **Amazon (AGI / Science)** | Applied Scientist Intern | Model efficiency, production 배포 경험 | Pure research보다 applied 성격 |
| **Qualcomm AI Research** | Research Intern | On-device AI, model compression. DynOPool의 efficiency 관점. | Mobile/edge 특화 필요 |
| **Samsung Research America** | Research Intern | 이전 Samsung 경험 + 네트워크 활용 가능 | 이미 Samsung 경험이 있으므로 다양성 관점에서 후순위 |

## Tier 3: 추천 (Niche Fit 또는 성장 기회)

| 회사 | 팀/포지션 | 매칭 이유 | 비고 |
|------|-----------|-----------|------|
| **Stability AI** | Research Intern | Diffusion model 전문. RewardFlow 직접 연관 | 스타트업 특성상 mentor 질 편차 |
| **Runway** | Research Intern (Video AI) | Video generation. PyraTok 연관 | 작은 팀, 높은 임팩트 가능 |
| **ByteDance Research** | Research Intern (Visual Generation) | PixelDance, MagicAnimate 등 video gen 팀 | 비자 이슈 확인 필요 |
| **Databricks (Mosaic ML)** | Research Intern | 이전 초청 강연 인연. Model efficiency 관점. | 주로 LLM 중심이라 CV fit 약함 |
| **Salesforce Research** | Research Intern (BLIP team) | Vision-language, generative model 경험 | 규모는 작지만 연구 질 높음 |
| **Toyota Research Institute** | Research Intern (ML) | 3D/4D vision for autonomous driving | 응용 분야가 다소 다름 |
| **Bosch AI** | Research Intern | Industrial AI + CV. Samsung 경험 활용 가능 | 유럽 중심이라 미국 포지션 제한적 |
...
> **주의:** 2026 Summer의 경우 이미 2025년 가을부터 모집이 시작되었을 가능성이 높습니다.
> 현재 시점(2026년 3월)에서는 late openings나 rolling basis 포지션에 집중하는 것이 현실적입니다.
> 2027 Summer를 미리 준비하는 것도 전략적으로 고려하세요.
